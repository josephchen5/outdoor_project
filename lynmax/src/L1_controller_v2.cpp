...
L1Controller::L1Controller()
{
    ...
    pub_ = n_.advertise<geometry_msgs::Twist>("carL1/cmd_vel", 1);
...
void L1Controller::controlLoopCB(const ros::TimerEvent&)
{
    ...
    cmd_vel.linear.x = 0;
    cmd_vel.angular.z = baseAngle*3.1415926/180;
    ...
    if(goal_received)
    {
        ...
        if(foundForwardPt)
        {
            cmd_vel.angular.z = (baseAngle + getSteeringAngle(eta)*Angle_gain)*3.1415926/180;
            /*Estimate Gas Input*/
            if(!goal_reached)
            {
