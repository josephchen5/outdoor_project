...
class BaseControl:
    def __init__(self, baseId, device_port, baudrate, VtoRPM):
	    ...
	    rospy.loginfo("Communication success !")
            rospy.loginfo("Using L1_Controller's base control !!!!")
	...
	# ROS handler        
        self.sub = rospy.Subscriber('/carL1/cmd_vel', Twist, self.cmdCB, queue_size=10) 
    ...
    def timerOdomCB(self, event):
		...
		Vx=RPM/self.VtoRPM
        	...
        	msg.twist.covariance[0] = 0.05 #Vx Cov
                msg.twist.covariance[1] = 0.05 #Vy Cov
